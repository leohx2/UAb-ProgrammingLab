...
void CopiaStrComLimite(char *dest, char *orig, int limite, int tamanho_maximo)
{
  int i;

  if (limite >= tamanho_maximo)
    limite = tamanho_maximo - 1;
...
int PalindromoJaExiste(SPalindromos *list, char *pali)
{
  while (list)
  {
    if (strcmp(list->pali, pali) == 0)
      return 1;
    list = list->next;
  }
  return 0;
...
{
  char guarda_pali[MAXPalindromo];
  int i, j, str_size;

  str_size = strlen(s);

  for (i = 0; i + 3 <= str_size; i++)
  {
    for (j = 3; i + j <= str_size && j <= MAXPalindromo; j++)
    {
      if (i + j <= str_size && IsAPalindromo(s + i, j))
      {
        CopiaStrComLimite(guarda_pali, s + i, j, MAXPalindromo);
        if (!PalindromoJaExiste(s_pali, guarda_pali))
          s_pali = AdicionaEmOrdem(s_pali, j, guarda_pali);
      }
    }
  }
